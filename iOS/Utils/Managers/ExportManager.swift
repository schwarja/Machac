...
private extension ExportManager {
    ...
    func exportContent(forPerson person: Person) -> String {
        let people = RealmManager.shared.people(without: person)
        ...
        for item in person.items {
            ...
            csv.add(doubleColumn: item.value)
        ...
        csv.add(column: "")
        csv.add(doubleColumn: person.items.reduce(0, { $0 + $1.value }))
        for prs in people {
        ...
        for ratio in person.ratios {
            ...
            csv.add(doubleColumn: ratio.item?.value ?? 0)
            ...
            csv.add(doubleColumn: ratio.ratio * (ratio.item?.value ?? 0))
